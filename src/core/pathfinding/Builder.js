 ...
 */
 export class Builder {
  ...
  static buildZone(geometry, tolerance) {
    ...
    // and could probably be optimized. For example, construct portals while determining the neighbor graph.
  ...
  static _buildPolygonNeighbours(polygon, vertexPolygonMap) {
    ...
    // It's only necessary to iterate groups A and B. Polygons contained only in group C cannot share a >1 vertex with this polygon.
  ...
  static _buildPolygonsFromGeometry(geometry) {
    ...
    // Constructing the neighbor graph brute force is O(n²). To avoid that, create a map from vertices to the polygons that contain them, and use it while connecting polygons.
    // This reduces complexity to O(n*m), where 'm' is related to connectivity of the mesh.
  ...
  static _getSharedVerticesInOrder(a, b) {
    ...
    // It seems that we shouldn't have an a and b with <2 shared vertices here unless there's a bug in the neighbor identification code, or perhaps a malformed input geometry; 
    // 3 shared vertices is a kind of embarrassing but possible geometry we should handle
