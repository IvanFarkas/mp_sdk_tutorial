 ...
 * @return {number}
 */
Pathfinding.prototype.getGroup = (() => {
  const plane = new Plane();
  return (zoneID, position, checkPolygon = false) => {
    if (!this.zones[zoneID]) {
      return null;
    }
 ...
 */
Pathfinding.prototype.clampStep = (() => {
  ...
  return (startRef, endRef, node, zoneID, groupID, endTarget) => {
    const vertices = this.zones[zoneID].vertices;
    const nodes = this.zones[zoneID].groups[groupID];
